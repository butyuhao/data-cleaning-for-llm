...
class WMT2019(TranslationPair):
    def __init__(self, pair="zh-en", split="train", mix_prob=0.2, maximum_size=2000000) -> None:
        super().__init__(mix_prob=mix_prob)
        self.dataset = load_dataset("wmt19", pair, cache_dir="/nas-alinlp/butyuhao/Github/Open-Assistant/model/model_training/.cache")[split]
        self.src, self.tgt = pair.split("-")

    def __getitem__(self, index):
        row = self.dataset[index]
        row = row["translation"]
        if random.random() > 0.5:
            source = random.choice(TRANSLATION_PROMPT[self.tgt]).format(row[self.src])
            return (source, row[self.tgt])
        else:  # translating in reverse direction
            source = random.choice(TRANSLATION_PROMPT[self.src]).format(row[self.tgt])
            return (source, row[self.src])
